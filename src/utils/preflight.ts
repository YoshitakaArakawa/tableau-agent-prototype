...
export function preflightValidateQuery(q: any): string {
  try {
    ...
    for (let i = 0; i < filters.length; i++) {
      ...
      const hasField = !!(
        f.field &&
        typeof f.field === "object" &&
        typeof f.field.fieldCaption === "string" &&
        f.field.fieldCaption.length > 0
      );
      if (t !== "TOP" && !hasField) return `filters[${i}].field required`;
      if (t === "TOP") {
        if (typeof f.howMany !== "number") return `filters[${i}].howMany required`;
        if (!f.fieldToMeasure || typeof f.fieldToMeasure !== "object") {
          return `filters[${i}].fieldToMeasure required`;
        }
      } else if (t === "SET") {
        if (!Array.isArray(f.values) || f.values.length === 0) {
          return `filters[${i}].values required`;
        }
        ...
        if (qft === "RANGE" || qft === "MIN") {
          if (!f.minDate || !isRfc3339Date(f.minDate)) {
            return `filters[${i}].minDate must be RFC3339`;
          }
        }
        if (qft === "RANGE" || qft === "MAX") {
          if (!f.maxDate || !isRfc3339Date(f.maxDate)) {
            return `filters[${i}].maxDate must be RFC3339`;
          }
        }
        ...
        if (Array.isArray(f.values) && f.values.length) {
          for (const v of f.values) {
            if (!isRfc3339Date(v)) return `filters[${i}].values must be RFC3339 dates`;
          }
        }
        if (f.minDate && !isRfc3339Date(f.minDate)) {
          return `filters[${i}].minDate must be RFC3339`;
